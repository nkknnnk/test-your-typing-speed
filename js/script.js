...
function randomParagraph() {
    ...
    // getting random item from the paragraphs array, spitting all characters of it , adding each character insite span and then adding this span inside p
...
function initTyping() {
    ...
    if (charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        // if user hasn't entered any character or paressed backspace 
        if (typedChar == null) {
            charIndex--;

            // decrement mistaks only if the charIndex span cntains incorrect class
            ...
            if (characters[charIndex].innerText === typedChar) {
                // if user typed character and shown character matched then add the correct class else increment the mistaks and the incorrect class
                characters[charIndex].classList.add("correct");
            } else {
                mistakes++;
                characters[charIndex].classList.add("incorrect");
            }
            charIndex++;  // increment charIndex either user typed correct or incorrect charactor
        }
